...
import { Map as MapIcon, Search, LightbulbOff, ArrowUpFromLine, ArrowDownToLine } from 'lucide-react';
import { useObstacleFilters, useObstacleData } from '@/context/ObstacleContext';
...
export default function Sidebar() {
  // Consumer: Filter Context (volatile) -> re-renders on slider/search interaction
  const {
    filteredData,
    avgCeiling,
  ...
  // Consumer: Data Context (stable) -> rarely updates
  const { stats } = useObstacleData();

  const obstacleCount = filteredData?.features.length || 0;

  // Memoize chart data to avoid array reallocation on each render
  const verticalData = useMemo(() => {
    if (!stats?.vertical) return [];
  ...
  const topFirs = useMemo(() => {
    return stats?.by_fir?.slice(0, 3) || [];
  }, [stats?.by_fir]);
  ...
  return (
    <div className="absolute top-0 left-0 h-full w-80 bg-slate-900/95 backdrop-blur-xl border-r border-slate-800 text-white flex flex-col shadow-2xl z-[1000]">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8 scrollbar-hide" data-version="v2">
          ...
          <div className="grid grid-cols-2 gap-2">
            {OBSTACLE_TYPES.map(type => (
              <button
                ...
                className={`px-3 py-2 rounded-md text-xs font-medium transition-all duration-200 border flex items-center gap-2 justify-center ${activeTypes.includes(type.id)
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                // Edge Case: Dragging to extreme right implies "Unlimited" (FL 500+)
